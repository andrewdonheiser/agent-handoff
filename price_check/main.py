...
def run_status_line():
    ...
    if today_period:
        print(_fmt_period_line(today_period, "Today"))

    for key, label in [("week_by_model", "Week"), ("month_by_model", "Month")]:
        period = state.get(key, {})
        if period:
            total_cost = sum(m.get("cost") or 0 for m in period.values())
            total_tok = sum(m.get("tokens") or 0 for m in period.values())
            print(f"{label}: {fmt_cost(total_cost)} ({fmt_tokens(total_tok)})")
